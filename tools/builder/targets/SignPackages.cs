...
public static class SignPackages
{
    ...
    public static async Task OnExecute(BuildContext context)
    {
        ...
        {
            context.WriteLineColor(ConsoleColor.Yellow, "Skipping packing signing because environment variables 'SignClientUser' and/or 'SignClientSecret' are not set.");
            return;
        }

        var signClientFolder = Path.Combine(context.BaseFolder, "packages", $"SignClient.{SignClientVersion}");
        if (!Directory.Exists(signClientFolder))
        {
            context.BuildStep($"Downloading SignClient {SignClientVersion}");

            await context.Exec(context.NuGetExe, $"install SignClient -version {SignClientVersion} -SolutionDir \"{context.BaseFolder}\" -Verbosity quiet -NonInteractive");
        }
